...
it("exposes 18 clickable, keyboard-selectable regional boundaries", async () => {
  ...
  const region = screen.getByRole("button", { name: /region 04, lower synthetic demo signal/i });
  region.focus();
  await user.keyboard("{Enter}");

  expect(screen.getAllByRole("button", { name: /region \d+.*synthetic demo signal/i })).toHaveLength(18);
